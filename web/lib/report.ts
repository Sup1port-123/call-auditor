...
async function sendReportEmail(opts: {
  ...
  const smtpHost = process.env.SMTP_HOST; // e.g. smtp-relay.brevo.com
  const smtpPort = parseInt(process.env.SMTP_PORT || "587", 10);
  const from = process.env.REPORT_FROM_EMAIL || smtpUser;

  if (smtpUser && smtpPass) {
    const { createTransport } = await import("nodemailer");
    // Use SMTP_HOST (Brevo / SendGrid / etc.) when set; fall back to Gmail.
  ...
  if (!apiKey) {
    throw new Error(
      "No email transport configured. Set SMTP_USER + SMTP_PASS (+ SMTP_HOST for Brevo/SMTP) or RESEND_API_KEY.",
    );
  }
