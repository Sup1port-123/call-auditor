        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <SummaryCard
              ...
              sub={`${Math.round((data.completed / data.total) * 100)}% success`}
              color="text-green-600"
            />
            <SummaryCard
              label="Failed / Pending"
              value={notCompleted.toLocaleString("en-IN")}
              sub={`${Math.round((notCompleted / data.total) * 100)}% of total`}
              color="text-red-500"
            />
            <SummaryCard
              label="Avg Score"
              value={scoreDisplay}
              sub={`${data.avgScore?.toFixed(2) ?? "-"} / 10`}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-6 mt-2">
                      ...
                      <div>
                        <div className="text-xl font-extrabold tabular-nums text-amber-700">
                          {Math.round(audioMin).toLocaleString("en-IN")} min
                        </div>
...
function SummaryCard({
  ...
  return (
    <div className="rounded-2xl bg-[var(--paper)] p-4">
      <div className={`font-display text-2xl font-extrabold tabular-nums ${color}`}>
        {value}
      </div>
